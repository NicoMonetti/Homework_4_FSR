...
% Plot results
figure(1)
plot(T, Y(:,1), 'b', 'LineWidth', 2,'DisplayName', '\theta (rad)');
hold on;
plot(T, Y(:,2), 'r', 'LineWidth', 2, 'DisplayName', '\theta dot (rad/s)');
xlabel('Time (s)');
ylabel('State');
...
figure(2)
plot(Y(:,1), Y(:,2), 'b', 'LineWidth', 2, 'DisplayName', '\theta (rad)');
hold on
